...
while True:
    ...
    if pressedKey == ord('q'):
        ...
        tempo = time.time()
        results = model(imagem_bgr,device = 'cpu',retina_masks=True, save = True, save_crop = True,save_frames=True,overlap_mask=True, project =r"C:\Users\labga\OneDrive\Documentos\IC_Julia\PROJETO_IC_IFES_BICO_DE_LANCA\Versao_3\resultados",name = nome, save_txt = True, show_boxes=False)
        for result in results:
            ...
            for j in range (detections):
                ...
                for i in range(len(depth_data_numpy_coordenada)): #para o bico de lança
                    ...
                    depth_data_numpy_binaria[j][x][y] = ((math.tan(Abertura*180/math.pi)*v*2)/640)
                    
                    #teste = np.sqrt(np.sum(depth_data_numpy_binaria[j])/pi_value
            
            # Exibir os diametros
            area_total = np.sum(depth_data_numpy_binaria)
            #print(area_total)
            diametro_total = 2*(math.sqrt(area_total/math.pi))
            area_furos = np.sum(depth_data_numpy_binaria[1:7],axis=(1,2))
            diametro_furos = 2*(np.sqrt(area_furos/math.pi))
            print(diametro_furos)
            print(diametro_total)
